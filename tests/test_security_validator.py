...
def test_blocks_eval():
    artifact = ArtifactBundle(files=[
        GeneratedFile(path="bad.py", content="result = eval('1+1')")
    ])
...
def test_blocks_exec():
    artifact = ArtifactBundle(files=[
        GeneratedFile(path="bad.py", content="exec('print(1)')")
    ])
    issues = SecurityValidator().validate(artifact)
    assert any(i.level == "error" for i in issues)
...
def test_allows_safe_code():
    artifact = ArtifactBundle(files=[
        GeneratedFile(path="safe.py", content="x = 1 + 2\nprint(x)")
    ])
    issues = SecurityValidator().validate(artifact)
...
def test_flags_eval_substring_in_safe_eval():
    ...
    """
    artifact = ArtifactBundle(files=[
        GeneratedFile(path="calc.py", content="result = _safe_eval('1+1')")
    ])
    issues = SecurityValidator().validate(artifact)
...
def test_skips_non_python():
    artifact = ArtifactBundle(files=[
        GeneratedFile(path="config.yaml", content="eval: true")
    ])
    issues = SecurityValidator().validate(artifact)
    assert len(issues) == 0
