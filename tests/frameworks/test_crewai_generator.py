...
def test_crewai_plain_python():
    code = Gen().generate_code(_dummy_workflow(), Settings(watsonx_api_key="x", watsonx_project_id="y"), mcp=False)
    assert "CrewAgent" in code and "CrewTask" in code


def test_crewai_mcp_wrapper():
    code = Gen().generate_code(_dummy_workflow(), Settings(watsonx_api_key="x", watsonx_project_id="y"), mcp=True)
